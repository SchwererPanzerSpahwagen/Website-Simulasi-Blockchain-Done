...
// ======== Render Konsensus Chain (REVISED) ========
function renderConsensusChains() {
  ["A", "B", "C"].forEach((u) => {
    ...
    chainsConsensus[u].forEach((blk, i) => {
      ...
      // Style merah jika invalid
      d.className = "chain-block" + (blk.invalid ? " invalid" : "");

      // HTML Template Baru (Hash Compact, Nonce & Time sejajar)
      d.innerHTML = `
        <h4>Block #${blk.index}</h4>
        
        <div style="margin-bottom:4px;">
           <label style="font-size:0.7rem; color:#888;">Data:</label>
           <textarea class="data" rows="2">${blk.data}</textarea>
        </div>

        <div class="meta-group">
          <div class="meta-item">
            <label>Nonce</label>
            <input value="${blk.nonce}" readonly>
          </div>
          <div class="meta-item">
            <label>Time</label>
            <input value="${blk.timestamp.split(' ')[1] || blk.timestamp}" readonly title="${blk.timestamp}">
          </div>
        </div>

        <div class="hash-group">
          <label>Prev Hash:</label>
          <input class="hash-input" value="${blk.prev}" readonly>
        </div>
        
        <div class="hash-group" style="margin-top:6px;">
          <label>Hash:</label>
          <input class="hash-input" value="${blk.hash}" readonly>
        </div>
      `;

      // Event listener agar manipulasi data terdeteksi
